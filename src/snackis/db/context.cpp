...
namespace snackis::db {
  ...
  optional<Error> Context::open(string path) {
    if (sqlite3_open(path.c_str(), &db)) { return Error("Failed opening database: ", sqlite3_errmsg(db)); }
    return nullopt;
  }
  ...
  optional<Error> Context::execute(string sql) const {
    ...
    if (sqlite3_exec(db, sql.c_str(), nullptr, 0, nullptr)) {
      return Error("Failed executing statement: ", sqlite3_errmsg(db), "\n", sql);
    }
